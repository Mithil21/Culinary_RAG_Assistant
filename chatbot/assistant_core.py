...
# You are a routing assistant. You have to understand the user intent. Classify the user's input into A, B, or C.
# A: ANY recipe request, general food query , or conversational reply ("yes", "okay").
# B: ONLY listing raw ingredients (e.g., "milk, sugar, rice").
# C: Explicitly asking for a NON-South Asian cuisine or any dish which is NOT SOUTH ASIAN.
...
def build_retrieval_query(question: str, intent: str, extracted: Dict[str, Any]) -> str:
    ...
    if intent == "INGREDIENTS_ONLY" and ingredients:
        extra = f" quick {time_preference}" if time_preference else ""
        return f"South Asian dish recipe using {' '.join(ingredients)}{extra}"

    return f"South Asian recipe {question}"
...
def classify_intent_node(state: GraphState):
    ...
    1. Rule-based first for robustness
    2. Optional LLM fallback only if needed
    """
    question = state["question"].strip()

    intent = rule_based_intent(question)
    extracted = extract_basic_slots(question)
...
def route_logic(state: GraphState) -> str:
    ...
    if intent == "INGREDIENTS_ONLY":
        # You can choose retrieval directly if you prefer.
        # For now, clarify first because ingredients-only often needs preference narrowing.
        return "clarify"

    if intent == "VAGUE_REQUEST":
        return "clarify_vague"

    if intent in {"RECIPE_REQUEST", "DISH_QUERY", "SUGGESTION_REQUEST", "CHAT_REPLY"}:
        return "retrieve"
